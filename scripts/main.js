...
(function($, undefined) {
  ...
  var FilterView = function() {
    ...
    function initialize() {
      ...
      $sortBy = $("#sortBy");
      shade =
        $themeColor.find('input[type="radio"]:checked').val() === "Dark"
          ? "dark"
          : "light";
  ...
  var GalleryView = function(initialFilter) {
    ...
    function loadAllThemes() {
      $.each(themeProperties.themes, function(i, theme) {
        $.each(themeProperties.languages, function(j, language) {
          ...
          if (
            (initialFilter.shade === "dark") != theme.dark ||
            initialFilter.language != language.name
          ) {
    ...
    function bindEvents() {
      $(document)
        .off("onShadeChange onLanguageChange")
        .on("onShadeChange onLanguageChange", function(
          event,
          language,
          shade,
          sortBy
      ...
      $(document)
        .off("onSortByChange")
        .on("onSortByChange", function(event, language, shade, sortBy) {
          sortThemes(sortBy);
        });
    }
  };
  ...
  var ThemeView = function(theme, language, shade) {
    ...
    function getThemeLocation() {
      var themeLocation =
        (theme.dark
          ? themeProperties.darkFolder
          : themeProperties.lightFolder) +
        theme.name +
        "/" +
        language.extension +
        themeProperties.imgExtension;
    ...
    function getThemeLinkMarkup() {
      ...
      if (theme.location != "native") {
        ...
        $div.append($a);
        $span = $(
          '<span class="downloads"> Downloads: ' +
